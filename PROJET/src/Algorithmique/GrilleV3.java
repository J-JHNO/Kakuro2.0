...
import Graphique.Interaction;
import Graphique.JFloatField;
import Graphique.Interaction.envoyerListener;
...
//					tabCase[i][j] = null;
	...
	public void valCaseBlanche() {
		for (int i=0; i<n1; i++) {
			for (int j=0; j<n2; j++) {
				if (tabCase[i][j].getType().equals("blanche")) {
					System.out.println(i + " " + j);
					tabText[i][j].setText(String.valueOf(((CaseBlanche) tabCase[i][j]).getValeur()));
				}
			}
		}
	}
	...
	public GrilleV3(int n1, int n2, ArrayList<Somme> ls, Case[][] tc) {
		setN1(n1);
		setN2(n2);
		initTab(n1,n2);
		listeSomme = ls;
		...
		if (n1>n2){
			...
			frame.setSize((50*n1+60),(40*n2+55));
			panel.setSize((50*n1+60),(40*n2+55));
		}
